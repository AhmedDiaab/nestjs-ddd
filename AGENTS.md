...
- **Metrics**: inject `MetricsPortToken` and use a name from `Metrics` (`infrastructure/metrics/metric-names.ts`), declaring it in `PrometheusMetrics` first. Labels must be bounded — never an id, URL or free text ([Observability](docs/architecture/observability.md)).
