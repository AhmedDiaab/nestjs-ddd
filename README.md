...
| **Observability**          | Prometheus metrics for HTTP in and out, jobs and pools (off by default), W3C trace context continued across services, and trace ids on every log line                                                                                                                                            |
...
| **Cluster mode**           | optional multi-core scaling on one box via Node's built-in `cluster` module (`CLUSTER_ENABLED`, off by default); the primary forks/respawns workers, elects a scheduler leader, forwards shutdown signals explicitly, and boot-time rails catch single-process assumptions before they run wrong |
